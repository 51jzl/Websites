    ...
    internal class QQAuthenticationHandler : AuthenticationHandler<QQAuthenticationOptions>
    {
        ...
        protected override async Task<AuthenticationTicket> AuthenticateCoreAsync()
        {
            ...
            try
            {
                ...
                string openidAddress = Options.OpenIdEndpoint + "?access_token=" + Uri.EscapeDataString(accessToken);
                if (Options.SendAppSecretProof)
                {
                    openidAddress += "&appsecret_proof=" + GenerateAppSecretProof(accessToken);
                }

                HttpResponseMessage openidResponse = await _httpClient.GetAsync(openidAddress, Request.CallCancelled);
                openidResponse.EnsureSuccessStatusCode();
                text = await openidResponse.Content.ReadAsStringAsync();
                string openid = GetParmFromContent(text, @"""openid"":""(?<openId>[^""]+)""", "openId");
                ...
                var context = new QQAuthenticatedContext(Context, user, accessToken, expires);
                ...
                {
                    context.Identity.AddClaim(new Claim("urn:QQ:name", context.Name, XmlSchemaString, Options.AuthenticationType));

                    // Many QQ accounts do not set the UserName field.  Fall back to the Name field instead.
                    if (string.IsNullOrEmpty(context.UserName))
                    {
                ...
                {
                    context.Identity.AddClaim(new Claim("urn:QQ:link", context.Link, XmlSchemaString, Options.AuthenticationType));
                }
